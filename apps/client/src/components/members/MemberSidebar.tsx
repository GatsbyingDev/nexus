...
export const MemberSidebar = ({ serverId }: MemberSidebarProps) => {
  const { data } = useServerMembers(serverId);
  const members = (data ?? []) as MemberShape[];
  ...
  return (
    <aside className="w-[240px] border-l border-outline-variant bg-surface-container p-3">
      ...
      <p className="mb-2 mt-4 text-xs font-semibold uppercase tracking-wide text-on-surface-variant">
